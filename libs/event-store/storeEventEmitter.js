...
function StoreEventEmitter(eventstore) {
  ...
  function enhanceCallback(name, callback, args) {
    ...
    callback = function () {
      self.eventstore.emit.call(
        self.eventstore,
        'after-' + name,
        getEmitArguments(args)
      )
  ...
  StoreEventEmitter.prototype.addEventEmitter = function () {
    ...
    if (this.store.clear) {
      ...
      this.store.clear = function (callback) {
        return addEventEmitterToMethod.call(
          this,
          'clear',
          originalClear,
          [],
    ...
    if (this.eventstore.getEventsSince) {
      ...
      this.eventstore.getEventsSince = function (
        date,
        skip,
        limit,
        callback
    ...
    if (this.eventstore.getEventsByRevision) {
      ...
      this.eventstore.getEventsByRevision = function (
        query,
        revMin,
        revMax,
        callback
        ...
        return addEventEmitterToMethod.call(
          this,
          'get-events-by-revision',
          originalGetEventsByRevision,
          [query, revMin, revMax],
          callback
        )
      }
    ...
    if (this.eventstore.getUndispatchedEvents) {
      var originalGetUndispatchedEvents =
        this.eventstore.getUndispatchedEvents
    ...
    if (this.eventstore.setEventToDispatched) {
      var originalSetEventToDispatched =
        this.eventstore.setEventToDispatched
      this.eventstore.setEventToDispatched = function (id, callback) {
        return addEventEmitterToMethod.call(
          this,
          'set-event-to-dispatched',
          originalSetEventToDispatched,
          [id],
          callback
        )
      }
    ...
    if (this.store.getPendingTransactions) {
      var originalGetPendingTransactions =
        this.store.getPendingTransactions
      this.store.getPendingTransactions = function (callback) {
        return addEventEmitterToMethod.call(
          this,
          'get-pending-transactions',
          originalGetPendingTransactions,
          [],
          callback
        )
      }
    ...
    if (this.store.repairFailedTransaction) {
      var originalRepairFailedTransaction =
        this.store.repairFailedTransaction
      this.store.repairFailedTransaction = function (lastEvt, callback) {
        return addEventEmitterToMethod.call(
          this,
          'repair-failed-transactions',
          originalRepairFailedTransaction,
          [lastEvt],
          callback
        )
      }
    ...
    if (this.eventstore.streamEventsByRevision) {
      var originalStreamEventsByRevision =
        this.eventstore.streamEventsByRevision
      this.eventstore.streamEventsByRevision = function (
        query,
        revMin,
        revMax
      ) {
        return addEventEmitterToMethod.call(
          ...
          originalStreamEventsByRevision,
          [query, revMin, revMax]
        )
      }
    }
    ...
    if (this.eventstore.getEventStream) {
      ...
      this.eventstore.getEventStream = function (
        query,
        revMin,
        revMax,
        callback
    ...
    if (this.eventstore.getFromSnapshot) {
      ...
      this.eventstore.getFromSnapshot = function (
        query,
        revMax,
        callback
      ) {
    ...
    if (this.eventstore.getLastEventAsStream) {
      var originalGetLastEventAsStream =
        this.eventstore.getLastEventAsStream
      this.eventstore.getLastEventAsStream = function (query, callback) {
        return addEventEmitterToMethod.call(
          this,
          'get-last-event-as-stream',
          originalGetLastEventAsStream,
          [query],
          callback
        )
      }
