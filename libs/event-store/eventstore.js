    ...
    async.waterfall(
      [
        ...
        function commitEvents(id, callback) {
          ...
          self.store.getNextPositions(
            uncommittedEvents.length,
            function (err, positions) {
              if (err) return callback(err)

              for (
                var i = 0, len = uncommittedEvents.length;
                i < len;
                i++
              ) {
              ...
              self.store.addEvents(uncommittedEvents, function (err) {
                ...
                if (self.publisher && self.dispatcher) {
                  ...
                  eventstream.eventsToDispatch = [].concat(
                    uncommittedEvents
                  )
                }
                ...
                eventstream.events =
                  eventstream.events.concat(uncommittedEvents)
