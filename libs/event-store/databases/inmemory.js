...
_.extend(InMemory.prototype, {
  ...
  addEvents: function (events, callback) {
    ...
    this.undispatchedEvents[context] =
      this.undispatchedEvents[context] || {}
    ...
    this.undispatchedEvents[context][aggregate][aggregateId] =
      this.undispatchedEvents[context][aggregate][aggregateId] || []
    this.undispatchedEvents[context][aggregate][aggregateId] =
      this.undispatchedEvents[context][aggregate][aggregateId].concat(
        events
      )
    ...
    if (!query.context && query.aggregate) {
      for (var s in this.store) {
        ...
        if (c[query.aggregate] && c[query.aggregate][query.aggregateId]) {
          if (revMax === -1) {
            res = res.concat(
              c[query.aggregate][query.aggregateId].slice(revMin)
            )
          } else {
            res = res.concat(
        ...
        for (var sa in cont) {
          ...
          if (agg[query.aggregateId]) {
            if (revMax === -1) {
              ...
              res = res.concat(
                agg[query.aggregateId].slice(revMin, revMax)
              )
            }
          }
        }
  ...
  addSnapshot: function (snap, callback) {
    ...
    this.snapshots[context][aggregate] =
      this.snapshots[context][aggregate] || {}
  ...
  getSnapshot: function (query, revMax, callback) {
    ...
    if (revMax === -1) {
      return callback(
        null,
        all[0] ? jsondate.parse(JSON.stringify(all[0])) : null
      )
    } else {
