...
_.extend(Elastic.prototype, {
  ...
  _buildQuery: function (type, find, sort, skip, limit) {
    var query = {
      ...
      size:
        !limit || limit === -1
          ? this.options.maxSearchResults - (skip || 0)
          : limit
    }
  ...
  _searchEvents: function (find, skip, limit, callback) {
    this._search(
      this.options.eventsTypeName,
      find,
      ['commitStamp:asc', 'streamRevision:asc', 'commitSequence:asc'],
      skip,
      limit,
      callback
    )
  ...
  _searchSnapshots: function (find, skip, limit, callback) {
    this._search(
      this.options.snapshotsTypeName,
      find,
      ['revision:desc', 'version:desc', 'commitStamp:desc'],
      skip,
      limit,
      callback
    )
  ...
  getLastEvent: function (query, callback) {
    ...
    this._search(
      this.options.eventsTypeName,
      findStatement,
      ['commitStamp:desc', 'streamRevision:desc', 'commitSequence:desc'],
      0,
      1,
      function (error, response) {
  ...
  getEventsByRevision: function (query, revMin, revMax, callback) {
    ...
    if (revMax === -1) {
      ...
      findStatement.push(
        'streamRevision:[' + revMin + ' TO ' + revMax + '}'
      )
    }
    findStatement.push('aggregateId:' + query.aggregateId)
    if (query.aggregate) findStatement.push('aggregate:' + query.aggregate)
  ...
  getUndispatchedEvents: function (query, callback) {
    ...
    if (query && query.aggregate)
      findStatement.push('aggregate:' + query.aggregate)
    if (query && query.context)
      findStatement.push('context:' + query.context)
  ...
  cleanSnapshots: function (query, callback) {
    ...
    this._searchSnapshots(
      ...
      function (error, response) {
        ...
        self._bulkDelete(
          self.options.snapshotsTypeName,
          response,
          callback
        )
      }
