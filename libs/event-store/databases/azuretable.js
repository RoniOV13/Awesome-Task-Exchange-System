...
_.extend(AzureTable.prototype, {
  connect: function (callback) {
    ...
    var server = azure
      .createTableService(
        this.options.storageAccount,
        this.options.storageAccessKey,
        this.options.storageTableHost
      )
    ...
    var createEventsTable = function (callback) {
      self.client.createTableIfNotExists(
    ...
    async.concat(
      ...
      function (event, callback) {
        var results = [
          ...
          function (callback) {
            var storedUndispatchedEvent = new StoredUndispatchedEvent(
              event
            )
            self.client.insertEntity(
              self.options.undispatchedEventsTableName,
              storedUndispatchedEvent,
              callback
            )
          }
        ]
  ...
  getEvents: function (query, skip, limit, callback) {
    ...
    tableQuery = _(query)
      ...
      .reduce(function (result, val, key) {
        ...
        if (result._where.length === 0)
          return tableQuery.where(key + ' eq ?', val)
  ...
  getEventsByRevision: function (query, revMin, revMax, callback) {
    ...
    if (revMax != -1)
      tableQuery = tableQuery.and('streamRevision < ?', revMax)
  ...
  getUndispatchedEvents: function (query, callback) {
    ...
    if (query && query.aggregate)
      tableQuery.where('aggregate eq ?', query.aggregate)
    if (query && query.context)
      tableQuery.where('context eq ?', query.context)
    if (query && query.aggregateId)
      tableQuery.where('aggregateId eq ?', query.aggregateId)
