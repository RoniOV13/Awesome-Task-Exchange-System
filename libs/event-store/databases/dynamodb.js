...
_.extend(DynamoDB.prototype, {
  connect: function (callback) {
    ...
    self.documentClient = new aws.DynamoDB.DocumentClient({
      service: self.client
    })
    ...
    var createSnapshotTable = function (callback) {
      createTableIfNotExists(
        self.client,
        SnapshotTableDefinition(self.options),
        callback
      )
    }
    ...
    var createUndispatchedEventsTable = function (done) {
      if (self.options.useUndispatchedEventsTable) {
        createTableIfNotExists(
          self.client,
          UndispatchedEventsTableDefinition(self.options),
          done
        )
      } else {
        done()
      ...
      function (event, callback) {
        var results = [
          function (callback) {
            var storedEvent = {
              ...
              ConditionExpression:
                'attribute_not_exists(aggregateId) and attribute_not_exists(streamRevision) and attribute_not_exists(context) and attribute_not_exists(#name)'
            }

            debug(
              'Saving event to events table: ' +
                JSON.stringify(storedEvent, null, 2)
            )
        ...
        if (self.options.useUndispatchedEventsTable) {
          ...
          results.push(function (callback) {
            ...
            debug(
              'Saving event to undispatchedevents table ' +
                JSON.stringify(storedEvent, null, 2)
            )
            self.documentClient.put(storedEvent, function (err, data) {
              if (err) {
                error(
                  'dynamodb.addUndispatchedEvents error: ' +
                    JSON.stringify(err)
                )
                return callback(err)
              } else {
  ...
  getEvents: function (query, skip, limit, callback) {
    ...
    if (query && query.context) {
      ...
      if (
        tableQuery.FilterExpression &&
        tableQuery.FilterExpression.length !== 0
      )
        tableQuery.FilterExpression += ' and context = :c'
      else tableQuery.FilterExpression = 'context = :c'
    }
    ...
    async.doWhilst(
      function (end) {
        if (exclusiveStartKey)
          tableQuery.ExclusiveStartKey = exclusiveStartKey
      ...
      function () {
        return entities.length < pageSize || pageSize == -1
  ...
  getEventsByRevision: function (query, revMin, revMax, callback) {
    ...
    if (revMax !== -1) {
      tableQuery.FilterExpression =
        'streamRevision BETWEEN :rmin AND :rmax'
  ...
  getLastEvent: function (query, callback) {
    ...
    async.doWhilst(
      ...
      function (err) {
        ...
        entities = _.sortBy(entities, function (e) {
          return [
            new Date(e.commitStamp).getTime(),
            e.streamRevision,
            e.commitSequence
          ]
        }).reverse()
  ...
  addSnapshot: function (snap, callback) {
    ...
    var snapshot = {
      ...
      ConditionExpression:
        'attribute_not_exists(aggregateId) and attribute_not_exists(id)'
    }
  ...
  getSnapshot: function (query, revMax, callback) {
    ...
    if (query && query.context) {
      ...
      if (
        ...
        tableQuery.FilterExpression += ' AND context = :ctx'
      else tableQuery.FilterExpression = 'context = :ctx'
    }
    ...
    if (revMax != -1) {
      ...
      else tableQuery.FilterExpression = 'revision <= :rmax'
      tableQuery.ExpressionAttributeValues[':rmax'] = revMax
    }
...
var StoredEvent = function (event) {
  debug(
    'Converting event to StoredEvent: ' + JSON.stringify(event, null, 2)
  )
  ...
  this.rowKey =
    ...
    (event.aggregate || '') +
    ':' +
    _.padStart(event.streamRevision, 16, '0')
  this.id = event.id
      ...
      if (err) {
        if (err.code === 'ResourceNotFoundException') {
          ...
          error(
            'Table ' +
              p.TableName +
              " doesn't exist yet but describeTable: " +
    ...
    client.describeTable({ TableName: name }, function (err, data) {
      if (err) {
        if (err.code === 'ResourceNotFoundException') {
          ...
          error(
            'deleteTableIfExists - describeTable error: ' +
              JSON.stringify(err, null, 2)
          )
          cbExists(err)
        }
        ...
        cbExists(null, {
          exists: true,
          description: { TableName: data.Table.TableName }
        })
      }
    })
  }
        ...
        client.describeTable(
          ...
          function (err, data) {
            if (err) {
              ...
              error(
                "Error calling describeTable for '" +
                  d.Table.TableName +
                  "'"
              )
              cbUntil(err)
...
var clearEventTables = function (opts, documentClient, cleared) {
  ...
  var maps = [
    {
      ...
      keyMap: function (n) {
        return {
          DeleteRequest: {
            Key: { aggregateId: n.aggregateId, rowKey: n.rowKey }
          }
        }
      }
    }
  ...
  var write = function (batch, callback) {
    if (batch && batch.RequestItems) {
      ...
      documentClient.batchWrite(batch, function (err, result) {
        if (err) {
          error(
            'Clear (batchWrite) error): ' + JSON.stringify(batch, null, 2)
          )
          return callback(err)
        }
  ...
  async.eachSeries(
    ...
    function (err) {
      if (err) {
        error(
          'Error while clearning events tables: ' +
            JSON.stringify(err, null, 2)
        )
        return cleared(err)
      }
