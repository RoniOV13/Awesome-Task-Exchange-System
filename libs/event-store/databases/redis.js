...
_.extend(Redis.prototype, {
    ...
    this.heartbeatInterval = setInterval(function () {
      var graceTimer = setTimeout(function () {
        if (self.heartbeatInterval) {
          console.error(
            new Error(
              'Heartbeat timeouted after ' + gracePeriod + 'ms (redis)'
            ).stack
          )
          self.disconnect()
        }
  ...
  getNewId: function (callback) {
    this.client.incr(
      'nextItemId:' + this.options.prefix,
      function (err, id) {
        if (err) {
          debug(err)
          return callback(err)
  ...
  addEvents: function (events, callback) {
    ...
    var prefix =
      self.options.prefix + ':' + self.options.eventsCollectionName
    var revisionKey =
      prefix +
      ':' +
      context +
    ...
    multi.exec(function (error, revisions) {
      ...
      if (errors.length) {
        var message =
          'error while adding events for aggregate ' +
          aggregate +
          ' ' +
          aggregateId
      ...
      var undispatchedKeysAndEvents = events.map(function (event) {
        var key =
          self.options.prefix +
          ':undispatched_' +
          self.options.eventsCollectionName +
          ':' +
          eventKey(event)
        return [key, JSON.stringify(event)]
      })
  ...
  getEvents: function (query, skip, limit, callback) {
    ...
    this.scan(
      this.options.prefix +
        ':' +
        this.options.eventsCollectionName +
        ':*:*:' +
        context +
        ':' +
        aggregate +
    ...
    this.scan(
      ...
      function (err) {
        ...
        evts = _.sortBy(evts, function (s) {
          return (
            s.commitStamp.getTime() + ':' + s.commitSequence.toString()
          )
        })
  ...
  addSnapshot: function (snap, callback) {
    ...
    this.client.set(
      this.options.prefix +
        ':' +
        this.options.snapshotsCollectionName +
        ':' +
  ...
  getSnapshot: function (query, revMax, callback) {
    ...
    this.scanSnapshots(query, function (err, allKeys) {
      ...
      if (revMax === -1) {
        // by default the last snapshot is kept
