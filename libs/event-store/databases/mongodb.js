...
function streamEventsByRevision(
  self,
  findStatement,
  revMin,
  revMax,
  ...
  findStatement.streamRevision =
    revMax === -1 ? { $gte: revMin } : { $gte: revMin, $lt: revMax }
  ...
  async.during(
    ...
    function (error) {
      ...
      var txOk =
        ...
        (revMax !== -1 &&
          (lastEvent.streamRevision === revMax - 1 ||
            !lastEvent.restInCommitStream))
    ...
    function initDb() {
      ...
      function finish(err) {
        ...
        self.events[ensureIndex](
          { aggregateId: 1, streamRevision: 1 },
          function (err) {
            if (err) {
              debug(err)
            }
        ...
        self.snapshots = self.db.collection(
          options.snapshotsCollectionName
        )
        self.snapshots[ensureIndex](
          { aggregateId: 1, revision: -1 },
          function (err) {
            if (err) {
              debug(err)
            }
        ...
        if (options.positionsCollectionName) {
          self.positions = self.db.collection(
            options.positionsCollectionName
          )
    ...
    this.heartbeatInterval = setInterval(function () {
      var graceTimer = setTimeout(function () {
        if (self.heartbeatInterval) {
          console.error(
            new Error(
              'Heartbeat timeouted after ' + gracePeriod + 'ms (mongodb)'
            ).stack
          )
          self.disconnect()
        }
  ...
  getNextPositions: function (positions, callback) {
    ...
    this.positions.findOneAndUpdate(
      ...
      function (err, pos) {
        ...
        callback(
          null,
          _.range(pos.value.position - positions, pos.value.position)
        )
      }
    )
  },
    ...
    this.events
      ...
      .toArray(function (err, res) {
        ...
        var txOk =
          ...
          (revMax !== -1 &&
            (lastEvt.streamRevision === revMax - 1 ||
              !lastEvt.restInCommitStream))
  ...
  repairFailedTransaction: function (lastEvt, callback) {
    ...
    this.transactions.findOne(
      ...
      function (err, tx) {
        ...
        if (!tx) {
          var err = new Error(
            'missing tx entry for aggregate ' + lastEvt.aggregateId
          )
          debug(err)
          return callback(err)
        }

        var missingEvts = tx.events.slice(
          tx.events.length - lastEvt.restInCommitStream
        )
