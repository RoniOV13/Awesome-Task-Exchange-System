...
_.extend(Tingo.prototype, {
  connect: function (callback) {
    ...
    // });
    this.events = this.db.collection(
      options.eventsCollectionName + '.tingo'
    )
    ...
    this.events.ensureIndex({ commitStamp: 1 }, function (err) {
      if (err) {
        debug(err)
      }
    })
    ...
    this.snapshots = this.db.collection(
      options.snapshotsCollectionName + '.tingo'
    )
    this.snapshots.ensureIndex(
    ...
    this.transactions = this.db.collection(
      options.transactionsCollectionName + '.tingo'
    )
